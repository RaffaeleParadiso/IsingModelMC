...
for ii in np.arange(10, 60, 10):
    ...
    plt.errorbar(beta_resc, susc_resc, yerr=err_susc*2*(1/ii**(7/4)), fmt='.', mfc='red',
...
for ii in np.arange(10,60,10):
    ...
    err_susc=np.loadtxt(f'{path}/{elem[6]}/sigma_susceptibility_lattice_dim_{ii}.txt')
    plt.errorbar(beta, susc, yerr=err_susc*2, fmt='.', mfc='red',
                  mec='green', ms=0.4, mew=2, capsize=2)
plt.show()
...
for ii in np.arange(10,60,10):
    ...
    beta_resc=np.loadtxt(f'{path}/{elem[1]}/{elem[1]}_lattice_dim_{ii}.txt')
    plt.errorbar(beta_resc, sh, yerr=err_sh*2, fmt='.', mfc='red',
                  mec='green', ms=0.4, mew=2, capsize=2)
plt.show()
...
for ii in np.arange(10, 60,10):
    ...
    beta_resc=np.loadtxt(f'{path}/{elem[1]}/{elem[1]}_lattice_dim_{ii}.txt')
    plt.errorbar(beta_resc, magn_resc, yerr=err_magn*2*ii**(1/8), fmt='.', mfc='red',
                  mec='green', ms=0.4, mew=2, capsize=2)
plt.show()
...
for ii in np.arange(10,60,10):
    ...
    err_magn=np.loadtxt(f'{path}/{elem[4]}/sigma_mean_magn_lattice_dim_{ii}.txt')
    plt.errorbar(beta, magn, yerr=err_magn*2, fmt='.', mfc='red',
                  mec='green', ms=0.4, mew=2, capsize=2) 
plt.show()
